---
title: 描边宽度 - Angular
description: 了解如何使用 `strokeWidth` 输入调整 Angular 应用中图标的描边宽度，或使用 `nonScalingStroke` 输入调整 strokeWidth 外观。
---
...
绑定 `size` 输入时，图标的描边宽度将相对于其大小，这是默认的 SVG 行为。引入 `nonScalingStroke` 输入是为了调整此行为，使描边宽度不受图标大小影响而保持不变。
...
### 使用 `nonScalingStroke` 输入调整描边宽度

将 `nonScalingStroke` 设置为 `true` 将使描边宽度不随大小缩放。
