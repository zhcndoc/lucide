...
它有意不包含**实际的渲染逻辑或组件**——但你可以使用此软件包为我们尚未支持的框架构建第三方集成。
...
| -------------------------- | ------------------ | ------------------------------------------------------- |
| `name`                     | `string`           | 图标的名称。                                            |
| `node`                     | `LucideIconNode[]` | 作为 `[tagName, attributes]` 元组的 SVG 子节点。        |
| `size` or `width` & `height` | `number`         | 图标的尺寸（`size` 是正方形图标的简写）。               |
...
| `nonScalingStroke` | `boolean`                | 将 [`vector-effect="non-scaling-stroke"`](https://developer.mozilla.org/en-US/docs/Web/SVG/Reference/Attribute/vector-effect) 添加到子元素。 |
...
此辅助函数同时适用于浏览器和 Node.js：

- 在浏览器中使用 `btoa`（并正确处理 UTF-8）
- 在 Node.js 中回退到 `Buffer`
...
::: tip 环境说明
SVG 在进行 base64 转换之前会编码为 UTF-8，以确保正确处理非 ASCII 字符。

无需运行时配置。该函数会自动选择适当的编码策略。

如果 `btoa` 和 `Buffer` 均不可用，则会抛出错误。
:::
